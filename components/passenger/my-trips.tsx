...
export function MyTrips({ onNewTrip }: MyTripsProps) {
  ...
  return (
    <div className="space-y-6">
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <p className="font-medium text-gray-800">{trip.origin}</p>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t">
                  ...
                  <div className="flex items-center gap-2 text-sm font-semibold text-[#1a5276]">
                    ...
                    <span>${trip.final_price.toFixed(2)} USD</span>
                  </div>
                </div>
                    ...
                    <div className="grid md:grid-cols-2 gap-4">
                      <div>
                        <p className="font-medium text-gray-800">
                          {trip.demo_driver.first_name} {trip.demo_driver.last_name}
                        </p>
                        <div className="flex items-center gap-1 mt-1">
                          <Star className="w-4 h-4 text-yellow-500 fill-yellow-500" />
                          <span className="font-medium">{trip.demo_driver.rating}</span>
                          <span className="text-gray-500 text-sm">
                            ({trip.demo_driver.total_trips} viajes)
                          </span>
                        </div>
                        <a 
                          href={`tel:${trip.demo_driver.phone}`}
                          className="flex items-center gap-1 text-[#1a5276] hover:underline mt-2"
                        >
                          <Phone className="w-4 h-4" />
                          {trip.demo_driver.phone}
                        </a>
                      </div>
                      <div>
                        <p className="text-gray-600">
                          {trip.demo_driver.vehicle_brand} {trip.demo_driver.vehicle_model}
                        </p>
                        <p className="text-gray-600">
                          Color: {trip.demo_driver.vehicle_color}
                        </p>
                        <p className="font-mono text-gray-800 mt-1">
                          Placa: {trip.demo_driver.vehicle_plate}
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                <div className="flex flex-wrap gap-3 pt-4 border-t">
                  ...
                  {trip.status === "completed" && trip.demo_driver && (
                    <Button
                      variant="outline"
                      size="sm"
      ...
      {/* Review Modal */}
      {selectedTrip && selectedTrip.demo_driver && (
        <DriverReviewModal
          ...
          driver={selectedTrip.demo_driver}
