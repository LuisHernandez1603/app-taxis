...
export function TripConfirmation({ tripData, onNewTrip }: TripConfirmationProps) {
  ...
  const hasDriver = !!tripData.demoDriver
            ...
            <div className="flex flex-col md:flex-row items-center gap-6">
              ...
              <div className="flex-1 text-center md:text-left">
                <h3 className="text-xl font-bold text-[#1a5276]">
                  {tripData.demoDriver.first_name} {tripData.demoDriver.last_name}
                </h3>
                <div className="flex items-center justify-center md:justify-start gap-1 mt-1">
                  {[...Array(5)].map((_, i) => (
                    <Star 
                      key={i} 
                      className={`w-4 h-4 ${
                        i < Math.floor(tripData.demoDriver!.rating) 
                  ...
                  <span className="text-sm text-gray-600 ml-1">
                    {tripData.demoDriver.rating} ({tripData.demoDriver.total_trips} viajes)
                  </span>
                </div>
                <a href={`tel:${tripData.demoDriver.phone}`} className="inline-flex items-center gap-2 mt-2 text-[#1a5276] hover:text-amber-500 transition-colors">
                  <Phone className="w-4 h-4" />
                  {tripData.demoDriver.phone}
                </a>
                {tripData.demoDriver.current_location && (
                  <p className="text-sm text-gray-500 mt-1">
                    <MapPin className="w-3 h-3 inline mr-1" />
                    Actualmente en: {tripData.demoDriver.current_location}
                  </p>
                )}
              </div>
              ...
              <div className="bg-gray-50 rounded-lg p-4 text-center">
                ...
                <p className="font-semibold text-[#1a5276]">
                  {tripData.demoDriver.vehicle_brand} {tripData.demoDriver.vehicle_model}
                </p>
                <p className="text-sm text-gray-600">{tripData.demoDriver.vehicle_color}</p>
                <div className="mt-2 bg-[#1a5276] text-white px-4 py-2 rounded-lg font-mono text-lg font-bold">
                  {tripData.demoDriver.vehicle_plate}
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="border-t pt-4">
              ...
              {tripData.distanceKm && (
                <p className="text-sm text-gray-500 mt-2 ml-8">
                  Distancia: {tripData.distanceKm} km
                </p>
              )}
            </div>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 text-sm">
            <p className="text-amber-800">
              <strong>Nota:</strong> Si cancelas con menos de 2 horas de anticipacion, podria aplicarse un cargo por cancelacion tardía.
            </p>
          </div>
